...
open class SteppedProgressBar: UIView {
    ...
    @IBInspectable open var activeTextColor: UIColor = UIColor.black {
        didSet {
            self.setNeedsDisplay()
        }
    }
    ...
    open var titles = ["Confirmed", "On The Way","Received"] {
        didSet {
            self.setNeedsDisplay()
        }
    }
    ...
    override open func draw(_ rect: CGRect) {
        ...
        if currentTab == 0 {
            ...
            drawTabs(from: 0, to: numberOfItems, color: activeStepColor(currentTab), textColor: activeTextColor)
        }
        else {
            ...
            drawTabs(from: 0, to: currentTab , color: activeStepColor(currentTab), textColor: activeTextColor)
    ...
    func drawTabs(from begin: Int, to end: Int, color: UIColor, textColor: UIColor) -> (start: CGPoint, end: CGPoint) {
        ...
        path.lineWidth = lineWidth
        for i in begin..<end {
            draw(step: i, path: path, start : &point, color: color, textColor: textColor)
            if i != (end - 1) {
                //draw trailinng line
    ...
    func draw(step i: Int, path: UIBezierPath, start point: inout CGPoint, color: UIColor, textColor: UIColor) {
        ...
        attributes[NSAttributedString.Key.font] = UIFont.arial(type:.regular,size: 12)
