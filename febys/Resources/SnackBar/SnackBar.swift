...
open class SnackBar: UIView, SnackBarAction, SnackBarPresentable {
    ...
    private lazy var actionButton: UIButton = {
        ...
        actionButton.setTitleColor(
            style.actionTextColor.withAlphaComponent(style.actionTextColorAlpha),
            for: .normal)
    ...
    private func setupView() {
        self.setupSubview(mainStackView) {
            $0.makeConstraints {
                $0.leading.equalTo(self).offset(style.innerHorizontalPadding)
                $0.trailing.equalTo(self).offset(-style.innerHorizontalPadding)
                $0.top.equalTo(self).offset(style.innerVerticalPadding)
                $0.bottom.equalTo(self).offset(-style.innerVerticalPadding)
            }
        }
    }
